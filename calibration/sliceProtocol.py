...
def updateParticle(part, best, phi1, phi2):
    ...
    for i, speed in enumerate(part.speed):
        if speed < -part[i]:
            part.speed[i] = -part[i]/2
        elif speed > part.smax[i]:
            part.speed[i] = part.smax[i]
    part[:] = list(map(operator.add, part, part.speed))    
 
def eval(lst):
    result = sliceRoutine.detect(lst)
    fitness_total = 0
    data = [11.14] # Number of biopsies on average with Seattle Protocol
    for i in range(0,3):
        fitness_total = fitness_total + (abs(result[3]-result[i]))**2
    value = fitness_total/float(3)
    value = value + (float(result[5]-data[0])/float(100))**2
    fit = [value]
    return fit
...
toolbox.register("particle", generate, size=4, 
                 pmin=[2,2,4,2], pmax=[10,8,15,10], 
                 smin=[-0.2,-0.2,-0.5,-0.5], 
                 smax=[2,2,2,2])
...
def main():   
    ...
    for g in range(GEN):
        ...
        for part in pop:
            ...
            lst = list()
            for i in range(0,4): # 4 is the number of free parameters
                lst.append(part[i])
